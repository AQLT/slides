...
In \texttt{RJDemetra} you have \textbf{one function} to set the
specification (\texttt{regarima\_spec\_x13()},
\texttt{regarima\_spec\_tramo()}, \texttt{x13\_spec()} and
\texttt{tramoseats\_spec()}) now one function for each part of the
specification
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{siratioplot}\NormalTok{(mod)}
\end{Highlighting}
\end{Shaded}
...
Currently in \faIcon{r-project}, with \texttt{filter()} you can only
manipulate symmetric filters (\(p=f\), \(\theta_{-k}=\theta_{-k}\)) or
real-time asymmetric filters: \[
M_\theta(X_t)=
...
In practice, you often need to combine filters (e.g.~X-11) or use
different filters according to the number of future points availables
(see \texttt{?rjd3filters::finite\_filters} and
\texttt{?rjd3filters::jfilter})
\end{frame}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m2 }\OtherTok{=} \FunctionTok{moving\_average}\NormalTok{(}\FunctionTok{rep}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{3}\NormalTok{), }\AttributeTok{lags =} \SpecialCharTok{{-}}\DecValTok{1}\NormalTok{) }\SpecialCharTok{/} \DecValTok{3}\NormalTok{; m2 }\CommentTok{\# centered MA}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## [1] "0,3333 B + 0,3333 + 1,3333 F +  F^2 +  F^3"
\end{verbatim}
...
\begin{verbatim}
## [1] " - 0,3333 B - 0,3333 + 0,6667 F +  F^2 +  F^3"
\end{verbatim}
...
\begin{verbatim}
## [1] "0,3333 + 0,6667 F +  F^2 + 0,6667 F^3 + 0,3333 F^4"
\end{verbatim}
