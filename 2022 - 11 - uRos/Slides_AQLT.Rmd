...
Currently in `r fa_r_project`, with `filter()` you can only manipulate symmetric filters ($p=f$, $\theta_{-k}=\theta_{-k}$) or real-time asymmetric filters:
$$
M_\theta(X_t)=
...
In practice, you often need to combine filters (e.g. X-11) or use different filters according to the number of future points availables (see `?rjd3filters::finite_filters` and `?rjd3filters::jfilter`)
...
m2 = moving_average(rep(1,3), lags = -1) / 3; m2 # centered MA
m1 + m2
m1 - m2
m1 * m2
```
