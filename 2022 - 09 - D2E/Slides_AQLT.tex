...
\faArrowCircleRight{} avec \(b=h+1\) et \(K_p\) spécifique on retrouve
...
\faArrowCircleRight{} \(b\) choisit par optimisation, e.g.~minimisant
les révisions (\(b_{q,\Gamma}\)), les révisions liées à la fonction de
gain (\(b_{q,G}\)) et celles liées au déphasage (\(b_{q,\varphi}\))
\end{frame}
