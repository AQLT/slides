...
- Appliquent une moyenne mobile de 13 termes sur la série désaisonnalisée (Cohérent avec X-13-ARIMA-SEATS utilisée pour la CVS-CJO) : CLF (`clf_smoothing()`) et Henderson (`henderson_smoothing()`)
$$
...
- Entraine *de facto* des révisions concentrées sur les 3 derniers mois (mais sont moindres que celles sur la série désaisonnalisées) !
...
$$
\hat{TC}_{t}=\hat\beta_{0,t}=\sum_{j=-h}^{h}w_{j}y_{t+j}
$$
